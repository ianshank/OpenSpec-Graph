...
- **`openspec_graph/detect.py`** — `StackProfile` gains
  ...
  since SpecKit has no `changes/` nesting) and
  ...
  SpecKit fingerprint is content-gated, not a bare `is_dir()` check like
  `openspec_root`'s: `root / "specs"` must be a directory **and**
  `find_speckit_spec_files()` must find at least one `*/spec.md` beneath it
  — a bare `specs/` directory is too common a name (OpenAPI, RSpec,
  JSON-schema conventions all use it) to trust structurally alone. `profile()`
  unions both roots' discovered spec files before calling `detect_dialect()`;
  both roots can coexist on the same profile (a mid-migration repo).
- **`openspec_graph/dialect_card.py`** — `_COMPARABLE_FIELDS`, and
  `StackProfile.as_dict()`/`to_card()` in `detect.py`, gain `speckit_root`/
  `feature_dirs`, mirroring how `openspec_root`/`adr_source` are already
  exposed. No `SCHEMA_VERSION` bump — matches the `adr_source`/`adr_ids`
  precedent of an old card simply not comparing a field it predates
  (`diff_cards()`'s existing absent-key skip).
...
- **`openspec_graph/rules_speckit.py`** (new) — rule family prefix `S` (the
  ...
  | ID | Severity | Check |
  |---|---|---|
  | S001 | ERROR | Unresolved `[NEEDS CLARIFICATION]` marker present |
  | S002 | ERROR | Duplicate `FR-`/`SC-` identifier (mirrors H004's duplicate-AC precedent, broadened to also cover requirements — harness has no requirement-duplicate check; a deliberate departure, justified on its own merits) |
...
- **`openspec_graph/rules_generic.py`** — **mandatory fix, not optional
  polish.** `HARD_THRESHOLD = re.compile(r"(?:≥|>=|>)\s*\d{2,3}\s*%?|\b\d{2,3}\s*%")`
  ...
  and `THRESHOLD_ALLOWLIST` has no token that exempts it. G003 is ERROR and
  ...
  positive. Fix: **G002** narrows `dialects` from `("*",)` to
  `("harness", "upstream")` — explicit opt-out. **G003** exempts the
  `## Success Criteria` section body from the hard-coded-threshold scan when
  `dialect == "speckit"`, using the same blank-the-span-preserve-line-numbers
  technique `strip_waiver_comments()` already uses elsewhere in
  `parse_semantics.py`. Zero behavior change for harness/upstream (neither
  fixture has that heading).
