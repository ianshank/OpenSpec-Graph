...
| `rules.py` | Facade over the rule submodules (below); 22 deterministic rules + waiver handling | `Rule`, `Finding`, `evaluate`, `evaluate_tree` |
...
flowchart TD
    ...
    pkg --> rules["rules.py<br/>facade: RULES = GENERIC + HARNESS + UPSTREAM + WITNESS"]
...
`rules_upstream.py` covers U001-U005, and `rules_witness.py` covers
W001-W002 — dialect-agnostic vs. per-dialect vs. opt-in witness-mode rule
