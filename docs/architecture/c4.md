...
`evaluate_tree()` checks (G006 and G009) entirely, while `graph --change`
always includes them, unscoped, regardless of what's rendered (DEC-GV-002,
generalized to G009 by DEC-AD-004) — so the two can disagree on a
`--change`-scoped run specifically because of those checks.
`StackProfile.to_card()` (`dialect_card.py`'s
...
├── detect.py                # StackProfile: languages, make_targets, dialect, invariants, adrs
