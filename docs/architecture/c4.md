...
`detect` + `parse` + `rules.evaluate`/`evaluate_tree` calls, so
`broken_links` always equals the finding count (AC-GR-4) — they cannot
disagree by construction. `StackProfile.to_card()` (`dialect_card.py`'s
